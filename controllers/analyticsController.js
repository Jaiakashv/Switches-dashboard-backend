const { getCpuUsageByRange } = require('../services/cpuDataService')
...
// CPU usage endpoint with time range support - now uses real data from Redis
const getCpuUsage = async (req, res, next) => {
  try {
    const { range = '24h' } = req.query
    
    // Get real CPU data from Redis
    const cpuData = await getCpuUsageByRange(range)
